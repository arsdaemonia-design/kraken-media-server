...
def init_db():
    """Initializes the database, creating it and migrating from JSON if necessary."""
    os.makedirs(config.DOWNLOAD_FOLDER, exist_ok=True)
    
    # Create default media subdirectories
    default_subdirs = ['Musica', 'OSTs', 'Video']
    for subdir in default_subdirs:
        os.makedirs(os.path.join(config.DOWNLOAD_FOLDER, subdir), exist_ok=True)
    
    # Create OSTs subdirectories
    osts_dir = os.path.join(config.DOWNLOAD_FOLDER, 'OSTs')
    for ost_subdir in ['Anime OST', 'Video Game Music']:
        os.makedirs(os.path.join(osts_dir, ost_subdir), exist_ok=True)
